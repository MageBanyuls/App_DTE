...
class ENVIODTE:
    def make_envio(self, documentos_dte, infoemisor, infoiddoc):
        ...
        tipo_dte_count = defaultdict(int)

        for dte in dtes:
            infoiddoc = dte.find('.//IdDoc')
            tipo_dte = infoiddoc.find('TipoDTE').text
            tipo_dte_count[tipo_dte] += 1

        print(tipo_dte_count)

        for tipo_dte, count in tipo_dte_count.items():
            subtotdte = etree.SubElement(caratula, "SubTotDTE")
            tpodte = etree.SubElement(subtotdte, "TpoDTE")
            tpodte.text = tipo_dte
            nrodte = etree.SubElement(subtotdte, "NroDTE")
            nrodte.text = str(count)
        ...
        with open(cert_pem_path, "rb") as cert_file:
            ...
            cert_lines = cert_pem.decode("ISO-8859-1").splitlines()
            cert_data = "".join(cert_lines[1:-1])
            x509_certificate_node_text = base64.b64encode(base64.b64decode(cert_data)).decode('ISO-8859-1')
